...
BEGIN
    ALTER TABLE [supplier] ADD company_id BIGINT NULL;
END
...
BEGIN
    ALTER TABLE [supplier] ALTER COLUMN company_id BIGINT NOT NULL;
END
...
IF NOT EXISTS (
    ...
    WHERE TABLE_NAME = 'supplier'
      AND CONSTRAINT_NAME = 'FK_supplier_company_id_company_id'
)
BEGIN
    ALTER TABLE [supplier]
    ADD CONSTRAINT FK_supplier_company_id_company_id FOREIGN KEY ([company_id]) REFERENCES [company](Id);
END
...
IF NOT EXISTS (
    SELECT 1
    FROM sys.indexes
    WHERE name = 'IX_supplier_company_id'
