 ...
 */
 export class ApiService {
   ...
   */
    delete(path: string, data: any, showSpinner: boolean = true): Observable<any> {
   ...
   * Realiza un post a la api
   */
    async postAsync(path: string, data: any, showSpinner: boolean = true): Promise<any> {
      return this.httpClient.post(`${this.API_ENDPOINT}/${path}`, data).toPromise();;
    }
  
    getClob(path: string, filename: string, showSpinner: boolean = true, download: boolean = true): Observable<any> {
      return this.httpClient.get(`${this.API_ENDPOINT}/${path}`, { responseType: 'blob' as 'json' })
        .pipe(map((response: any) => {
          const dataType = response.type;
          const binaryData = [];
          binaryData.push(response);
          const downloadLink = document.createElement('a');
          var objectUrl = window.URL.createObjectURL(new Blob(binaryData, { type: dataType }));
    ...
    postClob(path: string, filename: string, body: any, showSpinner: boolean = true): Observable<any> {
      return this.httpClient.post(`${this.API_ENDPOINT}/${path}`, body, { responseType: 'blob' as 'json' })
        .pipe(map((response: any) => {
          const dataType = response.type;
          const binaryData = [];
          binaryData.push(response);
